...
class StyleGAN2(nn.Module):
    def __init__(self, image_size, latent_dim = 512, style_depth = 8, network_capacity = 16, steps = 1, lr = 1e-4):
        ...
        self.G_opt = hvd.DistributedOptimizer(
                Adam(generator_params, lr = self.lr, betas=(0.5, 0.9)),
                # compression=hvd.Compression.fp16,
        )
        self.D_opt = hvd.DistributedOptimizer(
                Adam(self.D.parameters(), lr = self.lr, betas=(0.5, 0.9)),
                # compression=hvd.Compression.fp16,
        )
        self._init_weights()
...
class Trainer():
    ...
    def set_data_src(self, folder):
        get_train_loader = get_dali_train_loader(
            dali_cpu=True, 
    ...
    def train(self):
        ...
        for i in range(self.gradient_accumulate_every):
            ...
            image_batch, _ = next(self.loader)
        ...
        if self.steps % 500 == 0 and hvd.rank() == 0:
            self.save(floor(self.steps / SAVE_EVERY))

        if self.steps % 1000 == 0 or (self.steps % 100 == 0 and self.steps < 2500) and hvd.rank() == 0:
...
if __name__ == "__main__":
    train_from_folder(batch_size=32, num_workers=20)
